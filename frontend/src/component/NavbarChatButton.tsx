...
import CantarellaIntro from '@/component/chat/Intro'; 
...
export default function NavbarChatButton() {
  ...
  const handleButtonClick = () => {
    if (isChatOpen) {
      // TRƯỜNG HỢP A: Chat đang mở -> Bấm để đóng lại ngay (Không cần Intro)
      dispatch(toggleChat());
    } else {
      // TRƯỜNG HỢP B: Chat đang đóng -> Bật Intro lên trước
      setShowIntro(true);
    }
  };
  const handleIntroComplete = () => {
    setShowIntro(false);     // Tắt màn hình Intro
    dispatch(toggleChat());  // Dispatch Redux để mở khung Chat lên
  };

  return (
    <>
      {/* --- PHẦN 1: NÚT BẤM TRÊN NAVBAR (GIỮ NGUYÊN GIAO DIỆN CỦA BẠN) --- */}
      <button
        onClick={handleButtonClick} // Đổi hàm xử lý tại đây
      ...
      {/* --- PHẦN 2: COMPONENT INTRO (HIỆN FULL MÀN HÌNH KHI KÍCH HOẠT) --- */}
      {showIntro && (
        <CantarellaIntro 
