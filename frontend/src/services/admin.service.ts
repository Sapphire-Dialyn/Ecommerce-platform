...
  },
  
  // --- HÀM MỚI BỔ SUNG: BAN USER ---
  /**
   * Cấm tài khoản người dùng theo thời gian cụ thể.
   * @param userId ID người dùng.
   * @param banUntil Ngày hết hạn cấm (ISO string).
   * @param reason Lý do cấm.
   */
  banUser: async (userId: string, banUntil: string, reason: string): Promise<void> => {
    // Giả sử API endpoint là POST hoặc PATCH đến /admin/users/{id}/ban
    // và nhận body là { banUntil: string, reason: string }
