...
export default function HomePage() {
  ...
  // SỬA 1: Thêm state quản lý danh mục lấy từ DB
  const [categories, setCategories] = useState<any[]>([]); 
  const [loading, setLoading] = useState(true);

  // SỬA 2: Mảng hình ảnh dự phòng (Giữ lại 3 tấm hình đẹp của bạn)
  const fallbackImages = [
    "https://www.laneige.com.vn/media/catalog/product/2/5/250624_final_vn_wb_hyaluronic_cleansing_foam_thumbnail_600x600.jpg",
    "https://file.hstatic.net/200000073977/article/my-pham-trang-diem_ffc49920bf144f08886d93fb22f15cbf.jpg",
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        // SỬA 3: Gọi song song cả API Products và Categories
    ...
    <main className="min-h-screen bg-white font-sans text-gray-800 selection:bg-fuchsia-200">
          ...
          <div className="flex flex-col-reverse md:flex-row items-center gap-8 lg:gap-16">
            ...
            <div className="w-full md:w-5/12 space-y-10 text-center md:text-left"> 
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start pt-4"> 
                <Link href="/shop/products" className="px-8 py-4 bg-gray-900 text-white rounded-full font-bold shadow-lg hover:bg-fuchsia-600 hover:shadow-fuchsia-500/30 transition-all transform hover:-translate-y-1 text-center">
                  Mua ngay
                </Link>
                <Link href="/about" className="px-8 py-4 bg-white text-gray-800 border border-gray-200 rounded-full font-bold shadow-sm hover:bg-fuchsia-50 hover:text-fuchsia-700 hover:border-fuchsia-200 transition-all transform hover:-translate-y-1 text-center">
                  Tìm hiểu thêm
                </Link>
              </div>
            ...
            <div className="w-full lg:w-7/12 relative">
                ...
                <div className="relative z-10 rounded-[3rem] overflow-hidden shadow-2xl border-[6px] border-white/60 group h-[550px] sm:h-[650px] md:h-[700px] lg:h-[650px]">
                  <img 
                    src="https://res.cloudinary.com/dvtklxato/image/upload/v1775021845/kxaioycdunxi94ydxfku.jpg" 
                    alt="Hero Product" 
      ...
      <section className="py-5 bg-pink-50 pb-2">
        <div className="container mx-auto px-10 flex flex-col items-center">
          ...
          {/* CỘT DANH MỤC (Đã được động hóa) */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 max-w-7xl w-full leading-20">
            {loading ? (
               <div className="col-span-3 text-center py-10 text-fuchsia-600 font-bold">Đang tải danh mục...</div>
            ) : categories.length === 0 ? (
               <div className="col-span-3 text-center py-10 text-gray-500">Chưa có danh mục nào. Hãy thêm từ trang Admin.</div>
            ) : (
              // SỬA 4: Lặp qua mảng categories từ Database (Lấy 3 cái đầu tiên cho đẹp khung hình)
              categories.slice(0, 3).map((cat, index) => (
                <Link 
                  href={`/shop/products?categoryId=${cat.id}`} // <--- ID chuẩn 100% từ Database
                  key={cat.id} 
                ...
                >
                  <div className="w-48 h-48 md:w-60 md:h-60 rounded-full p-2 border-4 border-dashed border-fuchsia-300 group-hover:border-fuchsia-600 transition duration-500 bg-white shadow-lg group-hover:shadow-fuchsia-200">
                    <div className="w-full h-full rounded-full overflow-hidden bg-white relative flex items-center justify-center">
                      <img 
                        // Dùng ảnh từ Database (nếu bạn có cột image), không thì tự động gán hình fallback theo index
                  ...
                  <span className="text-xl md:text-2xl font-bold text-gray-800 group-hover:text-fuchsia-700 transition font-serif uppercase text-center leading-snug">
                    {cat.name}
                  </span>
                </Link>
              ))
        ...
        <div className="container px-6 relative z-10"> 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-16 items-center">
                <div className="flex flex-col items-center text-center space-y-6 md:space-y-8 p-6 md:p-8 bg-white/70 backdrop-blur-sm rounded-xl shadow-lg">
                    ...
                    <Link href="/about" className="inline-flex items-center justify-center px-20 py-6 bg-white/70 text-gray-800 rounded-full font-bold text-lg shadow-xl hover:bg-fuchsia-600 hover:text-white transition transform hover:-translate-y-1">
                        Tìm hiểu thêm
                    </Link>
                </div>
            ...
            <div className="mt-10 md:mt-10 grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="relative rounded-xl overflow-hidden shadow-lg group">
                    <img src="https://i.pinimg.com/736x/3f/7f/41/3f7f41c17175bc619adc1dbec482d039.jpg" alt="Gallery Image 1" className="w-full h-72 object-cover transform group-hover:scale-105 group-hover:blur-sm transition-all duration-500"/>
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <span className="text-white font-medium bg-black/50 px-4 py-2 rounded-lg">Tri thức</span>
                    </div>
                </div>
                <div className="relative rounded-xl overflow-hidden shadow-lg group">
                    <img src="https://images.squarespace-cdn.com/content/v1/53883795e4b016c956b8d243/f1d313a6-9480-433d-acc9-14e7c201b6b4/20604667_1901002723259976_1653435101074820527_n.jpg" alt="Gallery Image 2" className="w-full h-72 object-cover transform group-hover:scale-105 group-hover:blur-sm transition-all duration-500"/>
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <span className="text-white font-medium bg-black/50 px-4 py-2 rounded-lg">Vui vẻ</span>
                    </div>
                </div>
                <div className="relative rounded-xl overflow-hidden shadow-lg group">
                    <img src="https://bloganchoi.com/wp-content/uploads/2023/10/aespa-ningning-1.jpg" alt="Gallery Image 3" className="w-full h-72 object-cover transform group-hover:scale-105 group-hover:blur-sm transition-all duration-500"/>
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
