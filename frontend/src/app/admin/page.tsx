...
import { 
  ...
  CheckCircle, ShieldCheck, Ban, Eye
} from 'lucide-react';
...
export default function AdminUserManagement() {
  ...
  // --- FILTER DATA ---
  useEffect(() => {
    setDisplayData(allUsers.filter(user => user.role === activeTab));
  }, [activeTab, allUsers]);

  // --- ACTIONS ---
  const handleVerify = async (id: string, type: 'sellers' | 'enterprise' | 'logistics') => {
    if(confirm("Xác nhận duyệt hồ sơ đối tác này?")) {
      try { await adminService.verifyAccount(id, type, true); fetchAllData(); alert("Đã duyệt thành công!"); }
      catch { alert("Lỗi khi duyệt"); }
    }
  };

  const handleToggleStatus = async (id: string, currentStatus: boolean) => {
    try { await adminService.toggleUserStatus(id, !currentStatus); fetchAllData(); }
  ...
  const TableHeader = ({ cols }: { cols: string[] }) => (
    <thead>
      <tr className="bg-gray-50 border-b border-gray-200 text-gray-500 text-sm uppercase tracking-wider">
        {cols.map((col, idx) => (
          <th key={idx} className={`py-4 px-6 font-semibold ${idx === cols.length - 1 ? 'text-right' : idx > 1 ? 'text-center' : 'text-left'}`}>{col}</th>
        ))}
      </tr>
    </thead>
  );
        ...
        {displayData.map(user => (
          <tr key={user.id} className="hover:bg-fuchsia-50/40 transition-colors duration-200">
            <td className="py-4 px-6 flex items-center gap-4">
              <img src={user.avatar || 'https://via.placeholder.com/40'} className="w-10 h-10 rounded-full object-cover border shadow-sm"/>
        ...
        {displayData.map(user => (
          <tr key={user.id} className="hover:bg-fuchsia-50/40 transition-colors duration-200">
            <td className="py-4 px-6 flex items-center gap-4">
              ...
              <div>
                <div className="font-bold text-gray-900">{user.seller?.storeName || 'Chưa đặt tên'}</div>
        ...
        {displayData.map(user => (
          <tr key={user.id} className="hover:bg-fuchsia-50/40 transition-colors duration-200">
            <td className="py-4 px-6 flex items-center gap-4">
              ...
              <div>
                <div className="font-bold text-gray-900">{user.enterprise?.companyName || user.name}</div>
      ...
      <tbody className="text-sm divide-y divide-gray-100">
        {displayData.map(user => (
          <tr key={user.id} className="hover:bg-fuchsia-50/40 transition-colors duration-200">
            ...
            <td className="py-4 px-6 text-center font-bold text-gray-800 text-base">{user.logistics?.baseRate ? `${user.logistics.baseRate.toLocaleString()}đ` : '---'}</td>
            <td className="py-4 px-6 text-center">
      ...
      <tbody className="text-sm divide-y divide-gray-100">
        {displayData.map(user => (
          <tr key={user.id} className="hover:bg-fuchsia-50/40 transition-colors duration-200">
            ...
            <td className="py-4 px-6 text-center text-yellow-500 font-bold text-base">{user.shipper?.rating ? `★ ${user.shipper.rating.toFixed(1)}` : '---'}</td>
            <td className="py-4 px-6 text-center">
  <span className={`px-3 py-1 rounded-full text-xs font-bold border ${
    user.isActive ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200'
  }`}>
      ...
      <main className="p-6 w-full">
        <div className="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden min-h-[600px] flex flex-col">
          <div className="flex items-center justify-start border-b border-gray-200 bg-gray-50/50 p-2 gap-4 overflow-x-auto sticky top-0 z-10">
            {['CUSTOMER','SELLER','ENTERPRISE','LOGISTICS','SHIPPER'].map(tab => {
              ...
              return (
                <button key={tab} onClick={()=>setActiveTab(tab as any)} className={`shrink-0 flex items-center justify-center gap-2 px-5 py-3 rounded-xl text-sm font-bold transition-all duration-300 ${active?'bg-fuchsia-100 text-fuchsia-700 shadow-sm ring-1 ring-fuchsia-200 translate-y-0':'text-gray-500 hover:bg-white hover:text-gray-700 hover:shadow-sm'}`}>
